...
"""
import sys
sys.path.append('/home/jundu/SE_MOT')
sys.path.append('/home/jundu/SE_MOT/models')
import clip
import copy
import math
import numpy as np
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        clip_model, self.preprocess = clip.load("RN50", device=self.device)
        # 只保留图像编码器
    ...
    def _forward_single_image(self, image_features, samples, track_instances: Instances, gtboxes=None):
        ...
        feature1 = self.batchnormal_c1(feature1)
        ...
        feature2 = self.batchnormal_c2(feature2)
        ...
        # fusion_feature = self.linear5(fusion_feature)
        # fusion_feature = self.relu(fusion_feature)
        # fusion_feature = self.linear6(fusion_feature)
        # fusion_feature = self.relu(fusion_feature)
        ...
        feature_contcat = self.batchnormal_c3(feature_contcat)
        ...
        fusion_feature = self.batchnormal_c4(fusion_feature)
        fusion_feature = self.relu(fusion_feature)
        fusion_feature = self.conv5(fusion_feature)
        fusion_feature = self.batchnormal_c5(fusion_feature)
        fusion_feature = self.relu(fusion_feature)
        fusion_feature = self.conv6(fusion_feature)
        fusion_feature = self.relu(fusion_feature)
        fusion_feature = fusion_feature.view(-1)
        fusion_feature = self.linear5(fusion_feature)
        fusion_feature = self.relu(fusion_feature)
